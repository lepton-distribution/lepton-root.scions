...
const char *yaffs_norif1_c_version = "$Id: yaffs_norif1.c,v 1.1 2010-05-31 13:44:39 phlb Exp $";
...
#define M18_SKIP		         16
#define PROG_REGION_SIZE	   512 // see _yaffs_readfs() parameters in yaffscore.c.
#define BLOCK_SIZE_IN_BYTES 	128*1024 //block size in Bytes
#define CHUNKS_PER_BLOCK	   240 // see _yaffs_readfs() parameters in yaffscore.c.
...
#define DATA_BYTES_PER_CHUNK	PROG_REGION_SIZE
#define BLOCKS_IN_DEVICE       (32*1024/128) //32MBytes flash size
...
//#undef __tauon_posix__

   #ifdef __tauon_posix__
      #include "kernel/fs/yaffs/yaffsdriver_nor_flash.h"
      #define ynorif1_FlashInit(__dev__) _yaffsdriver_nor_flash_initialise(__dev__)
      #define ynorif1_FlashDeinit(__dev__) _yaffsdriver_nor_flash_shutdown(__dev__)
      #define ynorif1_FlashWrite32(__dev__,__addr__,__buf__,__nwords__) _yaffsdriver_nor_flash_write32(__dev__,__addr__,__buf__,__nwords__) 
      #define ynorif1_FlashRead32(__dev__,__addr__,__buf__,__nwords__) _yaffsdriver_nor_flash_read32(__dev__,__addr__,__buf__,__nwords__)
      #define ynorif1_FlashEraseBlock(__dev__,__addr__) _yaffsdriver_nor_flash_eraseblock(__dev__,__addr__)
      #define DEVICE_BASE     (__u32)0
   #else
      #include "ynorsim.h"
      #define ynorif1_FlashInit(__dev__) ynorsim_Initialise()
      #define ynorif1_FlashDeinit(__dev__) ynorsim_Shutdown()
      #define ynorif1_FlashWrite32(__dev__,__addr__,__buf__,__nwords__) ynorsim_Write32(__addr__,__buf__,__nwords__)
      #define ynorif1_FlashRead32(__dev__,__addr__,__buf__,__nwords__) ynorsim_Read32(__addr__,__buf__,__nwords__) 
      #define ynorif1_FlashEraseBlock(__dev__,__addr__) ynorsim_EraseBlock(__addr__)
...
int ynorif1_WriteChunkToNAND(yaffs_Device *dev,int chunkInNAND,const __u8 *data, const yaffs_Spare *spare)
{
        ...
        if(data && spare)
        {
                ...
                ynorif1_FlashWrite32(dev,spareAddr,(__u32 *)&tmpSpare,sizeof(yaffs_Spare)/4);

                /* Write the data */            
                ynorif1_FlashWrite32(dev,dataAddr,(__u32 *)data,dev->param.totalBytesPerChunk / 4);
                ...
                ynorif1_FlashWrite32(dev,spareAddr,(__u32 *)&tmpSpare,sizeof(tmpSpare)/4);  

        } else if(spare){
                /* This has to be a read-modify-write operation to handle NOR-ness */

                ynorif1_FlashRead32(dev,spareAddr,(__u32 *)&tmpSpare,16/ 4);
                
                ynorif1_AndBytes((__u8 *)&tmpSpare,(__u8 *)spare,sizeof(yaffs_Spare));
                
                ynorif1_FlashWrite32(dev,spareAddr,(__u32 *)&tmpSpare,16/ 4);
        }
        else {
                YBUG();
        }
...
int ynorif1_ReadChunkFromNAND(yaffs_Device *dev,int chunkInNAND, __u8 *data, yaffs_Spare *spare)
{
	...
	if(data)
	{
		ynorif1_FlashRead32(dev,dataAddr,(__u32 *)data,dev->param.totalBytesPerChunk / 4);
	}
	
        if(spare)
        {
                ynorif1_FlashRead32(dev,spareAddr,(__u32 *)spare,16/ 4);
...
static int ynorif1_FormatBlock(yaffs_Device *dev, int blockNumber)
{
	...
	ynorif1_FlashEraseBlock(dev,blockAddr);
	ynorif1_FlashWrite32(dev,formatAddr,&formatValue,1);
...
static int ynorif1_IsBlockFormatted(yaffs_Device *dev, int blockNumber)
{
	...
	ynorif1_FlashRead32(dev,formatAddr,&formatValue,1);
