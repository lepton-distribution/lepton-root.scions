...
=============================================*/
static char* pmemory;
static long memory_size = DFLT_FILEROM_MEMORYSIZE;
static int fh = -1;

static const char memory[DFLT_FILEROM_MEMORYSIZE] = { 0 };
...
/*-------------------------------------------
| Name:filerom_win32_isolation_open
| Description:
| Parameters:
| Return Type:
| Comments:
| See:
---------------------------------------------*/
int filerom_win32_isolation_open(const char* path) {
	//
	if (fh == -1) {
      //
...
/*-------------------------------------------
| Name:filerom_win32_isolation_close
| Description:
| Parameters:
| Return Type:
| Comments:
| See:
---------------------------------------------*/
int filerom_win32_isolation_close(void) {
	//
	if (fh == -1)
		return -1;
	//
...
/*------------------------------------------ -
| Name: filerom_win32_isolation_write
| Description :
| Parameters :
| Return Type :
| Comments :
| See :
-------------------------------------------- - */
int filerom_win32_isolation_write(int* offset, const char* buf, int size) {
	int w;
	//
...
/*------------------------------------------ -
| Name: filerom_win32_isolation_read
| Description :
| Parameters :
| Return Type :
| Comments :
| See :
-------------------------------------------- - */
int filerom_win32_isolation_read(int* offset, char* buf, int size) {
	int r;
	//
	if (*offset >= memory_size)
		return -1;
...
/*------------------------------------------ -
| Name: filerom_win32_isolation_getsize
| Description :
| Parameters :
| Return Type :
| Comments :
| See :
-------------------------------------------- - */
int filerom_win32_isolation_getsize(void) {
	return memory_size;
}

/*------------------------------------------ -
| Name: filerom_win32_isolation_setsize
| Description :
| Parameters :
| Return Type :
| Comments :
| See :
-------------------------------------------- - */
int filerom_win32_isolation_setsize(int size) {
	int w;
	//
	_lseek(fh, 0, SEEK_SET);
	//
	...
	if ((w = _lseek(fh, memory_size, SEEK_SET)) == -1)
		printf("filerom0 creation failed");
	else
		printf("filerom0 creation size %u bytes ok.\n", w);
	//
	return w;
}

/*------------------------------------------ -
| Name: filerom_win32_isolation_erase
| Description :
| Parameters :
| Return Type :
| Comments :
| See :
-------------------------------------------- - */
int filerom_win32_isolation_erase(void) {
	_close(fh);
	//
