...
int dev_linux_filerom_open(desc_t desc, int o_flag){
   ...
   if(fh==-1) {
      ...
      if( (fh =
              cyg_hal_sys_open( ".fsrom.o",_O_RDWR|_O_CREAT|_O_EXCL|_O_SYNC,
         ...
         if( (fh =
                 cyg_hal_sys_open( ".fsrom.o",_O_RDWR|/*_O_TRUNC|*/ _O_EXCL|_O_SYNC,
                                   _S_IREAD|_S_IWRITE)) == -1 )
            return -1;
...
/*			if(( w = cyg_hal_sys_write(fh,memory,sizeof( memory ))) == -1 ){
   ...
   if(fh==-1) {
      ...
      if( (fh =
              _sys_open( ".fsrom.o",_O_RDWR|_O_CREAT|_O_EXCL|_O_SYNC,_S_IREAD|_S_IWRITE)) == -1 ) {
         ...
         _sys_close(fh);
         if( (fh =
                 _sys_open( ".fsrom.o",_O_RDWR|_O_TRUNC|_O_EXCL|_O_SYNC,_S_IREAD|_S_IWRITE)) == -1 )
            return -1;
