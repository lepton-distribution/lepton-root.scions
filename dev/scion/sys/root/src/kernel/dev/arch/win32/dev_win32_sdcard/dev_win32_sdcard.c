...
int dev_win32_sdcard_open(desc_t desc, int o_flag){
   //
	if (sdcard_win32_isolation_open(DFLT_SDCARD_FILE_PATH) < 0)
		return -1;
   //
   if(o_flag & O_RDONLY) {
   }
...
---------------------------------------------*/
int dev_win32_sdcard_close(desc_t desc){
   return sdcard_win32_isolation_close();
}
...
---------------------------------------------*/
int dev_win32_sdcard_read(desc_t desc, char* buf,int size){
   return sdcard_win32_isolation_read(&ofile_lst[desc].offset, buf, size);
}
...
---------------------------------------------*/
int dev_win32_sdcard_write(desc_t desc, const char* buf,int size){
   return sdcard_win32_isolation_write(&ofile_lst[desc].offset, buf, size);
}
...
int dev_win32_sdcard_seek(desc_t desc,int offset,int origin){
   ...
   switch(origin){
	   ...
	   case SEEK_END:
		  dev_current_addr=(unsigned long)sdcard_win32_isolation_getsize();
		  break;
   }
      ...
      //
	   case HDGETSZ: {
		  ...
		  if(!hdsz_p)
			 return -1;
        //
		  *hdsz_p = sdcard_win32_isolation_getsize();
	   }
	   break;
	   ...
	   case HDSETSZ: {
        ...
        //
		  if(!hdsz)
			 return -1;
        //
        sdcard_win32_isolation_setsize(hdsz);
		  //
	   }
	   break;
