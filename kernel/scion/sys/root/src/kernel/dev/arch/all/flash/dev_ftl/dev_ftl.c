...
#define __set_vector_sector(__sector_no__)   g_p_vector_sector_free[(__sector_no__)/ \
                                                                    8]|=(0x01<<((__sector_no__)%8))
#define __unset_vector_sector(__sector_no__) g_p_vector_sector_free[(__sector_no__)/ \
                                                                    8]&=(~(0x01<<((__sector_no__)%8)))
#define __is_free_vector_sector(__sector_no__)   ((g_p_vector_sector_free[(__sector_no__)/ \
                                                                          8]& \
                                                   (0x01<<((__sector_no__)%8))) ? 0 : 1)

#define __set_vector_tobe_erase_sector(__sector_no__)   g_p_vector_sector_tobe_erase[(__sector_no__) \
                                                                                     /8]|= \
   (0x01<<((__sector_no__)%8))
#define __unset_vector_tobe_erase_sector(__sector_no__) g_p_vector_sector_tobe_erase[(__sector_no__) \
                                                                                     /8]&= \
   (~(0x01<<((__sector_no__)%8)))
#define __is_vector_tobe_erase_sector(__sector_no__)   ((g_p_vector_sector_tobe_erase[( \
                                                                                         __sector_no__) \
                                                                                      /8]& \
                                                         (0x01<<((__sector_no__)%8))) ? 1 : 0)
   ...
   while(sector_addr<dev_sz) {
      if(!sector_sz || !((sector_offs)%sector_sz)) {
         ...
         for(sector_cluster_no=0; sector_cluster_no<(sector_cluster_no_max); sector_cluster_no++) {
            ...
            //set position
            if((r=
                   ofile_lst[desc_link].pfsop->fdev.fdev_seek(desc_link,sector_addr+
                                                              (sector_cluster_no*p_ftl->cluster_sz),
                                                              SEEK_SET))<0)
               return -1;

            //read cluster tag
            if((r=
                   ofile_lst[desc_link].pfsop->fdev.fdev_read(desc_link,(char*)&cluster_tag,
                                                              sizeof(cluster_tag_t)))!=
               sizeof(cluster_tag_t))
               return -1;
            ...
            if((cluster_tag.sector_tag&0x0f)==0x05) {
               ...
               if(sector_cluster_no==0) {
                  if(sector_cluster_no==0 && cluster_tag.sector_stat.erase==
                     TAG_SECTOR_TOBEERASED) {
                  ...
                  if(p_ftl->p_cluster_lst[_cluster_no].phy_cluster_addr !=FTL_INVALID_ADDR) {
                     ...
                     //read tag
                     if((r=
                            ofile_lst[desc_link].pfsop->fdev.fdev_seek(desc_link,
                                                                       p_ftl->p_cluster_lst[
                                                                          _cluster_no].
                                                                       phy_sector_addr,SEEK_SET))<0)
                        return -1;
                     //read cluster tag
                     if((r=
                            ofile_lst[desc_link].pfsop->fdev.fdev_read(desc_link,
                                                                       (char*)&prev_cluster_tag,
                                                                       sizeof(cluster_tag_t)))!=
                        sizeof(cluster_tag_t))
                        return -1;
                     ...
                     if(prev_cluster_tag.sector_stat.move==TAG_SECTOR_MOVERUNNING) {
                        ...
                        p_ftl->p_cluster_lst[_cluster_no].phy_cluster_addr = sector_addr+
                                                                             sector_cluster_no*
                                                                             p_ftl->cluster_sz;
               ...
               p_ftl->p_cluster_lst[_cluster_no].phy_cluster_addr = sector_addr+sector_cluster_no*
                                                                    p_ftl->cluster_sz;
               p_ftl->p_cluster_lst[_cluster_no].phy_sector_addr  = sector_addr;
               p_ftl->p_cluster_lst[_cluster_no].phy_sector_no    = sector_no;
               p_ftl->p_cluster_lst[_cluster_no].phy_sector_sz    = sector_sz;
...
int _ftl_get_free_sector(desc_t desc, cluster_no_t cluster_no){
   ...
   //
   p_sector_map =
      (flash_sector_t*)((dev_flash_t*)ofile_lst[desc_link].p)->p_flash_type->p_sector_map;
   ...
   while((++new_sector_no)!=current_sector_no) {
      ...
      if(__is_free_vector_sector(new_sector_no)) {
         ...
         while(p_sector_map[i].addr!=0xFFFFFFFF) {
            ...
            sector_no_t sector_no_limit_high =
               (sector_no_t)((p_sector_map[i+1].addr-p_sector_map[i].addr)/sector_sz)+sector_no;
         ...
         do {
            if((sector_cluster_no*new_sector_sz)<(DEV_SECTOR_SZ_MAX)) {
               ...
               if((r=
                      ofile_lst[desc_link].pfsop->fdev.fdev_seek(desc_link,new_sector_addr+
                                                                 (sector_cluster_no*
                                                                  p_ftl->cluster_sz),SEEK_SET))<0)
                  return -1;
               if((r=
                      ofile_lst[desc_link].pfsop->fdev.fdev_read(desc_link,(char*)&cluster_tag,
                                                                 sizeof(cluster_tag_t)))!=
               ...
               if(cluster_tag.sector_stat.erase == TAG_SECTOR_TOBEERASED) //erase this sector
                  _ftl_ioctl_flash(desc_link,HDCLRSCTR,new_sector_addr+
                                   ((new_sector_no+sector_cluster_no)*p_ftl->cluster_sz));
            ...
            p_ftl->p_cluster_lst[cluster_no+
                                 sector_cluster_no].phy_cluster_addr = new_sector_addr+
                                                                       sector_cluster_no*
                                                                       p_ftl->cluster_sz;
            p_ftl->p_cluster_lst[cluster_no+sector_cluster_no].phy_sector_addr  = new_sector_addr;
            p_ftl->p_cluster_lst[cluster_no+
                                 sector_cluster_no].phy_sector_no    = new_sector_no+
                                                                       (sector_no_t)
                                                                       sector_cluster_no;
            ...
            if((r=
               ...
               return -1;
            if((r=
                   ofile_lst[desc_link].pfsop->fdev.fdev_write(desc_link,(char*)&cluster_tag,
                                                               sizeof(cluster_tag_t)))!=
               sizeof(cluster_tag_t))
               return -1;

#ifdef FTL_LOG
            printf("new sector:[%04d] cluster[%03d] addr=0x%08x\r\n",new_sector_no,cluster_no+
                   sector_cluster_no,
                   p_ftl->p_cluster_lst[cluster_no+sector_cluster_no].phy_cluster_addr);
#endif
         } while( (sector_cluster_no = ((++sector_cluster_no)%sector_cluster_no_max))!=
                  sector_cluster_no_limit );
...
int _ftl_move_sector(desc_t desc, cluster_no_t cluster_no,addr_t current_phy_addr,int size){
   ...
   while(cb<(DEV_SECTOR_SZ_MAX)) {
      ...
      if(!cb) {
         ...
         if((r=
                ofile_lst[desc_link].pfsop->fdev.fdev_seek(desc_link,dest_sector_addr+cb,
                                                           SEEK_SET))<0)
            return -1;
         if((r=
                ofile_lst[desc_link].pfsop->fdev.fdev_read(desc_link,(char*)&cluster_tag,
         ...
         if((r=
                ofile_lst[desc_link].pfsop->fdev.fdev_seek(desc_link,src_sector_addr+cb,
                                                           SEEK_SET))<0)
            return -1;
         if((r=
                ofile_lst[desc_link].pfsop->fdev.fdev_read(desc_link,(char*)&cluster_tag,
...
int dev_ftl_read(desc_t desc, char* buf,int size){
   ...
   while(cb<size) {
      ...
      phy_addr       =  p_ftl->p_cluster_lst[cluster_no].phy_cluster_addr+sizeof(cluster_tag_t)+
                       cluster_offs;
