...
Global Declaration
=============================================*/
ufs_block_node_1_3_t ufs_block_node_1_3;
...
| See:
---------------------------------------------*/
int _ufs_drv_1_3_drvmakefs(desc_t desc,superblk_t* superblk)
{
   int cb=0;
   int _offset=0;
...
| See:
---------------------------------------------*/
int _ufs_drv_1_3_drvreadfs(desc_t desc, superblk_t* superblk)
{
   int cb=0;
   int _offset=0;
   ...
   }else if(!strcmp(buffer,UFS_SIGNATURE_1_3)) {
      //read block size
      superblk->blk_size = 32;
      //
      if(( cb = ofile_lst[desc].pfsop->fdev.fdev_read(desc,
...
| See:
---------------------------------------------*/
int _ufs_drv_1_3_drvwritefs(desc_t desc,superblk_t* superblk)
{
   int cb=0;
...
int _ufs_drv_1_3_writeblk(desc_t desc,char* pufs_blk,int block,int size)
{
   ...
   //to remove test
   //printf("_ufs_drv_1_3_writeblk size=%d\n",size);
...
/*--------------------------------------------
| Name:        _ufs_drv_1_3_drv_1_3_sync_sp_block
| Description:
| Parameters:  none
| Return Type: none
| Comments:
| See:
----------------------------------------------*/
int _ufs_drv_1_3_sync_sp_blk(mntdev_t* pmntdev,char* pufs_blkalloc,int block_byte){
...
/*--------------------------------------------
| Name:        _ufs_drv_1_3_sync_sp_node
| Description:
| Parameters:  none
| Return Type: none
| Comments:
| See:
----------------------------------------------*/
int _ufs_drv_1_3_sync_sp_node(mntdev_t* pmntdev,char * pufs_nodealloc,int node_byte){
   desc_t desc = pmntdev->dev_desc;
   //
...
#endif //#if __KERNEL_SUPPORT_UFS_DRIVER==__KERNEL_SUPPORT_UFS_DRIVER_1_3

/*============================================
| End of Source  : ufsdriver_1_3.c
==============================================*/
