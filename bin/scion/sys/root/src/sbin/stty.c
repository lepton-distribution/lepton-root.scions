...
static int column= 0, max_column=80;           /* Assume 80 character terminals. */


static void report(int flags);
static int option(char *opt, char *next);
static int match(char *s1, char *s2);
static void prctl(char c);
static speed_t long2speed (long num);
static long speed2long(unsigned long speed );
static void print_flags(unsigned long flags, unsigned long flag,
                        unsigned long def, char *string, int all);

static void output(char *s);
static void do_print_char(unsigned chr, unsigned def, char *name, int all);
static void do_print_num(unsigned num, unsigned def, char *name, int all);
static void set_saved_settings(char *opt);
static void set_control(int option, char *value);
static void set_min_tim(int option, char *value);
...
int stty_main(int argc,char *argv[])
{
   ...
   {
      fprintf(stderr, "%s: can't read ioctl parameters from stdin.\r\n",
              prog_name);
      exit(1);
   }
   ...
   {
      fprintf(stderr, "%s: can't set terminal parameters to stdin\r\n",prog_name);
      exit(1);
   }
   exit(0);
...
static int option(char *opt, char *next){
   ...
   if (match(opt, "ispeed")) {
      ...
      {
         fprintf(stderr, "invalid argument to ispeed: '%s'\r\n", next);
         return 1;
      }
   }
   ...
   if (match(opt, "ospeed")) {
      ...
      {
         fprintf(stderr, "invalid argument to ospeed: %s\n", next);
         return 1;
      }
   }
...
| See:
---------------------------------------------*/
static void do_print_char(unsigned chr,
                          unsigned def,
                          char *name,
                          int all){
   char line[20];
...
| See:
---------------------------------------------*/
static void do_print_num(unsigned num,
                         unsigned def,
                         char *name,
                         int all){
   char line[20];
...
static void set_min_tim(int option,char *value){
   ...
   //assert(option >= 0 && option < NCCS);
   termios.c_cc[option]= (cc_t)num;
}

/*===========================================
End of Source stty.c
=============================================*/
