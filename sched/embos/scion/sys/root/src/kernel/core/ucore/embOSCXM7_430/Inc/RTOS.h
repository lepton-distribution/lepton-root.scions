...
*/
  ...
  #define OS_ExtendTaskContext_TLS_VFP   OS_ExtendTaskContext_TLS
#endif
...
#define OS_VERSION_GENERIC    (43000u)  //  Generic version 4.30
...
//
  ...
  #define TRACE_ON_TASK_START_EXEC(TaskId)                                   if (OS_Global.pTrace != NULL) {OS_Global.pTrace->pfRecordTaskStartExec((TaskId)); }
  #define TRACE_ON_TASK_STOP_EXEC()                                          if (OS_Global.pTrace != NULL) {OS_Global.pTrace->pfRecordTaskStopExec(); }
  #define TRACE_ON_TASK_START_READY(TaskId)                                  if (OS_Global.pTrace != NULL) {OS_Global.pTrace->pfRecordTaskStartReady((TaskId)); }
  #define TRACE_ON_TASK_STOP_READY(TaskId, Para0)                            if (OS_Global.pTrace != NULL) {OS_Global.pTrace->pfRecordTaskStopReady((TaskId), (Para0)); }
...
typedef struct {
  ...
  void  (*pfRecordTaskCreate)        (unsigned int TaskId);
  void  (*pfRecordTaskStartExec)     (unsigned int TaskId);
  void  (*pfRecordTaskStopExec)      (void);
  void  (*pfRecordTaskStartReady)    (unsigned int TaskId);
  void  (*pfRecordTaskStopReady)     (unsigned int TaskId, unsigned int Reason);
  ...
  void  (*pfRecordTaskTerminate)     (unsigned int TaskId);
...
void                OS_EVENT_CreateEx     (OS_EVENT* pEvent, OS_EVENT_RESET_MODE ResetMode)  OS_SECTION_ATTRIBUTE;
