...
#define OS_VERSION_GENERIC (38400)       /*   Generic version 3.84 */
...
typedef void OS_ON_TERMINATE_FUNC(OS_CONST_PTR OS_TASK *);
...
#if OS_TRACKNAME
  OS_ROM_DATA const char* OS_GetTaskName(OS_CONST_PTR OS_TASK *);
#else
...
#define OS_DecRI()       { OS_ASSERT_DICnt();                                   \
                           if (--OS_DICnt==0) { /*lint !e548 No else required*/ \
                             OS_EI(); }} 
